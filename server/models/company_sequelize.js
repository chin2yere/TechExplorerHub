...
const create = (githubId, name, description, picture_url) => {
  const query =
    "INSERT INTO company ( githubId, name, description, picture_url) VALUES ($1, $2, $3, $4) RETURNING *";
  return pool.query(query, [githubId, name, description, picture_url]);
};

const update = (id, githubId, name, description, picture_url) => {
  const query =
    "UPDATE company SET githubId = $2, name = $3, description = $4, picture_url = $5 WHERE id = $1";
